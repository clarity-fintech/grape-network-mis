...
> Creator **Chandler William Ferguson** · Org [`clarity-fintech/grape-network-mis`](https://github.com/clarity-fintech/grape-network-mis)
...
**Grape Network MIS** is written entirely in **MIS** (Moniversive Invariant Static, `.mis`) — a
...
```bash
git clone https://github.com/clarity-fintech/grape-network-mis
cd grape-network-mis
...
| `bin/misc mis/kernel/GrapeNetworkMisKernel.mis --check --compact-letters` | Check a single module directly with the compiler. |

Direct compiler invocation (what the Makefile runs under the hood):

```bash
bin/misc mis/kernel/GrapeNetworkMisKernel.mis --check --compact-letters
...
```
mis/kernel/GrapeNetworkMisKernel.mis
mis/packs/access_verify.mis
mis/packs/community_seal.mis
mis/packs/membership.mis
mis/packs/role_bound.mis
mis/packs/token_gate.mis
...
mis/commands/GrapeNetworkMisCommandCatalog.mis
mis/commands/GrapeNetworkMisCommands.mis
mis/commands/GrapeNetworkMisNetworkBind.mis
...
`mis/kernel/GrapeNetworkMisKernel.mis` — the kernel declares the invariants the compiler enforces on every check:
...
// Title: Grape Network MIS Kernel

module GrapeNetworkMisKernel {
  ...
  invariant domain_bound: domain == grape_network;
  ...
  invariant pack_token_gate_flag: pack_token_gate_bound == true;
  invariant pack_membership_flag: pack_membership_bound == true;
  invariant pack_access_verify_flag: pack_access_verify_bound == true;
  invariant pack_community_seal_flag: pack_community_seal_bound == true;
  invariant pack_role_bound_flag: pack_role_bound_bound == true;
...
// Creator: Chandler William Ferguson
// Compile: bin/misc <file>.mis --check --compact-letters
// Title: Grape Network MIS · AccessVerify

module GrapeNetworkMisAccessVerify {
  invariant letter_hash_bound: letter_hash_root != @0;
  invariant deep_root_moniversive: deep_root == moniversive;
  invariant settlement_chain: chain_id == 1202;
  invariant settlement_network_clrty1: settlement_network == clrty_1;
  invariant kernel_is_misc: compiler_kernel == misc;
  invariant pack_access_verify_bound: pack_bound == true;
  invariant catalog_backlink: mis_catalog_bound == true;

  invariant access_verify_041: access_verify_041_bound == true;
  invariant access_verify_042: access_verify_042_bound == true;
  invariant access_verify_043: access_verify_043_bound == true;
  invariant access_verify_044: access_verify_044_bound == true;
  invariant access_verify_045: access_verify_045_bound == true;
  invariant access_verify_046: access_verify_046_bound == true;
  invariant access_verify_047: access_verify_047_bound == true;
  invariant access_verify_048: access_verify_048_bound == true;
```
...
```
CONNECTED clrty-1/1202 grape-network-mis
RPC https://rpc.clarity-fintech.com
MODULES mis/commands/GrapeNetworkMisCommands.mis
```
...
{
  "ok": true,
  "module": "GrapeNetworkMisBacklinkIndex",
  "kernel": "misc",
  "invariant_count": 9,
  "outcome_count": 4,
  "typed_letters": 1157,
...
*Reproduce:* `make check` → `"ok": true`, module `GrapeNetworkMisBacklinkIndex`, 9 invariants, 4 outcomes, 1157 typed letters.
...
- [clarity-fintech/grape-network-mis](https://github.com/clarity-fintech/grape-network-mis) — this repository
